    ...
    const fake_names= [{
      ...
      function latest_deposits(){
        ...
        for (i = 0; i < 5; i++) {
            ...
            var given_name = `${fake_names[getRandomInt(1,fake_names.length)].GivenName}`;
            switch (i) {
                case 0:
        ...
        for (i = 0; i < latest_deposits.length; i++) {
          var date = get_date();
          rows += `<tr><td scope="row">${latest_deposits[i].name}</td><td>$${latest_deposits[i].amount}</td><td>${date}</td></tr>`
          //rows += `<p><span class="span1">${latest_deposits[i].name}</span><span>$</span> ${latest_deposits[i].amount}</span><span>${date}</span><i><img src="image/48.gif"></i></p>`
       }
        return rows;
      }
      function latest_withdraws(){
        ...
        for (i = 0; i < latest_withdraws.length; i++) {
          var date = get_date();
         // rows += `<li><span class="span1">${latest_withdraws[i].name}</span><span>$</span> ${latest_withdraws[i].amount}</span><span>${date}</span><i><img src="image/48.gif"></i></li>`
          rows += `<tr><td>${latest_withdraws[i].name}</td><td>$${latest_withdraws[i].amount}</td><td>${date}</td></tr>`
       }
        return rows;
      }
      ...
      $('#ld_body').append(flatest_deposits);
      $('#lw_body').append(flatest_withdraws);
